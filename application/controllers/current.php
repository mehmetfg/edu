...
class current extends CI_Controller implements CrudModelInterface {
    ...
    public function getList()
    {
        ...
        $viewData->model=$this->db->get('current')->result();
        $viewData->createBtn="current/credit";
        $this->load->view("current/List", $viewData);
    ...
    public function credit()
    {
        $viewData=new stdClass();
        $viewData->model=null;
        $viewData->action="current/savechange?type=1&id=0";
        $this->load->view("current/credit",$viewData);
    }
    public function saveChange()
    {
        $name = $this->input->post("currentName");
        $mail=$this->input->post("currentMail");
        $data=array(
            "Name"=> $name,
            "Mail"=>$mail
        );

        saveChange("current",$data);
    ...
    public function  isActiveSetter(){
        ...
        $this->db->where("id", $id)->update("current", array("isActive" => $isActive));
