...
class dealer extends CI_Controller implements CrudModelInterface {
        ...
        public function getList()
        {
            ...
            $viewData->model=$this->db->get('dealer')->result();
            $viewData->createBtn="dealer/credit";
            $this->load->view("Dealer/List", $viewData);
        ...
        public function credit()
        {
            $viewData=new stdClass();
            $viewData->model=null;
            $viewData->action="dealer/savechange?type=1&id=0";
            $this->load->view("Dealer/credit",$viewData);
        }
        public function saveChange()
        {
            $name = $this->input->post("DealerName");
            $mail=$this->input->post("DealerMail");
            $data=array(
                "Name"=> $name,
                "Mail"=>$mail
            );

            saveChange("dealer",$data);
        ...
        $id          = $this->input->post("id");
         ...
         $this->db->where("id", $id)->update("dealer", array("isActive" => $isActive));
